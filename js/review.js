...
async function initReviewPage() {
  ...
  // Tampilkan affiliate disclosure hanya untuk review & list
  const disclosure = document.getElementById('affiliate-disclosure');
  if (disclosure) {
    disclosure.style.display = (type === 'review' || type === 'list') ? 'block' : 'none';
  }

  // Update link "Lihat semua" di related section
  const relatedLink = document.getElementById('related-see-all');
  if (relatedLink) {
    relatedLink.href = (type === 'news' || type === 'article') ? '/berita' : 'kategori.html';
  }
